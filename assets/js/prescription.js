document.addEventListener("DOMContentLoaded", function () {
    ...
    function renderPrescriptions(data) {
        ...
        if (data.active.length > 0) {
            ...
            const date = new Date(info.date_prescribed).toLocaleDateString("en-US", {
            ...
            html += `       </table><hr><div class='advice'><p class='label'>Advice Given:</p>`;
            const uniqueAdvice = [...new Set(data.active.map(item => item.advice))];
            uniqueAdvice.forEach(advice => {
                html += `<p class='adviceText'>${advice}</p>`;
            });

            html += `</div></div></div><div class='footer'><p>-</p></div></div></div>
                     <div class='downloadPrescriptionButton'>
                         <button id='downloadPrescriptionBtn'><i class='bx bxs-printer'></i> Print Prescription</button>
                     </div>`;
        } else {
            html += `<div class='activePrescription'><p class='title'>Active Prescription</p><p>No active prescriptions in the last 7 days.</p></div>`;
        }

        // --- Prescription History ---
        html += `<div class='prescriptionList'><p class='title'>Prescription History</p>`;
        if (data.history.length > 0) {
            data.history.forEach(item => {
                const date = new Date(item.date_prescribed).toLocaleDateString("en-US", {
                    year: "numeric", month: "long", day: "numeric"
                });
            ...
            html += `<div class='prescriptionHistory'><p class='nothing'>Not recent prescription</p></div>`;
        }
        html += `</div>`;
